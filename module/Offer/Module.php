...
use Offer\Model\Category;
use Offer\Model\CategoryTable;
use Offer\Model\Logs;
use Offer\Model\LogsTable;
use Offer\Model\Code;
use Offer\Model\CodeTable;
   ...
   // \Zend\Debug\Debug::dump($application);
    $this->auth = new AuthenticationService();
        ...
        if($this->auth->hasIdentity()){
            echo $this->userid = $this->auth->getIdentity();
        }
    }
    ...
    public function getServiceConfig()
    {
        return array(
            'factories' => array(
                'Offer\Model\CodeTable' =>  function($sm) {
                    $tableGateway = $sm->get('CodeTableGateway');
                    $table = new CodeTable($tableGateway);
                    return $table;
                },
                'CodeTableGateway' => function ($sm) {
                    $dbAdapter = $sm->get('Zend\Db\Adapter\Adapter');
                    $resultSetPrototype = new ResultSet();
                    $resultSetPrototype->setArrayObjectPrototype(new Code());
                    return new TableGateway('offer_code', $dbAdapter, null, $resultSetPrototype);
                },
                'Offer\Model\LogsTable' =>  function($sm) {
                ...
                'Offer\Model\CategoryTable' =>  function($sm) {

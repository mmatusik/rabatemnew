...
class EditController extends AbstractActionController
{
    ...
    public function editAction()
    {
        ...
        if($link) {
            ...
            $viewmodel->setVariable('name', $offer_info->name);
            $viewmodel->setVariable('old_price', $offer_info->old_price);
            $viewmodel->setVariable('new_price', $offer_info->new_price);
            $viewmodel->setVariable('keyworlds', $offer_info->keyworlds);
            $viewmodel->setVariable('disc', $offer_info->disc);
            $viewmodel->setVariable('worth', $offer_info->worth);
            $viewmodel->setVariable('how', $offer_info->how);
            $viewmodel->setVariable('category_id', $offer_info->id_category);
            $viewmodel->setVariable('recommended', $offer_info->recommended);
            if($offer_info->pass == NULL) {
                $type = 'code';
                $viewmodel->setVariable('code_per_person', $offer_info->code_per_person);
                $viewmodel->setVariable('count_code', $codeTable->countCode($link));
                
            } else {
                $type = 'pass';
                $viewmodel->setVariable('pass', $offer_info->pass);
            }
        ...
        if ($request->isPost()) {  
            ...
            $codeTable->updateCodeEdit($linkz, $link);
            if ($form->isValid()) {     
                $genlink = $offerTable->genOfferLink($request->getPost('keyworlds'));
                $linkz = $genlink.uniqid();
                $offer = array(
                ...
                $offerTable->updateOffer($offer, $link); //DODANIE OFERTY                               
                return $this->redirect()->toRoute('editoffer', array('action' => 'company', 'link' => $linkz, ));
            }
        }
    ...
    public function companyAction()
    {
        ...
        if ($request->isPost()) {  
            ...
            if ($form->isValid()) {               
                ...
                return $this->redirect()->toRoute('editoffer', array('action' => 'photo', 'link' => $link, 'id' => 1 ));               
            }
        }
